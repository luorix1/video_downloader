...
def create_heatmap(element, output_dir, count, channel):
    ...
    np.save(output_dir + '/' + channel + '_' + str(count) + '.npy', [x_axis, y_axis])
    count += 1
    return count
...
def download(url, output_dir, count, channel):
    ...
    for i in range(7):
      ...
      if len(elements) > 0:
        countAfter = create_heatmap(elements[0], output_dir+'/graph', count, channel)
        print(count, countAfter)
        if count == countAfter:
          return count
        yt = YouTube(url)
        stream = yt.streams.filter(res="360p", progressive="True")[0]
        stream.download(output_path=output_dir+'/video', filename=channel+"_"+str(count)+'.mp4')
...
if __name__ == '__main__':
  ...
  parser.add_argument('--output_dir', default='/data/data1/youtube',
  ...
  output_dir = args.output_dir + '/' + args.channel
  read_file = open('./' + args.channel + '.csv', 'r', encoding='utf-8')
  links = csv.reader(read_file)
  isExist = os.path.exists(output_dir)
  if isExist == False:
    os.makedirs(output_dir)
    os.makedirs(output_dir + '/video')
    os.makedirs(output_dir + '/graph')
  count = int(len(os.listdir(output_dir + '/graph')))
  for link in links:
    if int(link[1].split(args.channel + '_')[1]) < int(args.start):
      continue
    try:
      print(link)
      count_before = count
      count = download(link[0], output_dir, count, args.channel)
      if count_before != count:
        write_file = open('./' + args.channel + '_youtube_completed.csv', 'w', encoding='utf-8')
